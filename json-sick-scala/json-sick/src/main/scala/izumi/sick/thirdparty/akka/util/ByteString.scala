...
object ByteString {
  ...
  private class SerializationProxy(@transient private var orig: ByteString) extends Serializable {
    private def writeObject(out: ObjectOutputStream): Unit = {
      out.writeByte(orig.byteStringCompanion.SerializationIdentity.toInt)
...
sealed abstract class ByteString extends IndexedSeq[Byte] with IndexedSeqOps[Byte, IndexedSeq, ByteString] with StrictOptimizedSeqOps[Byte, IndexedSeq, ByteString] {
  ...
  @SuppressWarnings(Array("AvoidSizeEqualsZero"))
...
final class ByteStringBuilder extends Builder[Byte, ByteString] { builder =>
  ...
  @inline protected final def fillByteBuffer(len: Int, byteOrder: ByteOrder)(fill: ByteBuffer => Any): this.type = {
    fillArray(len) {
      case (array, start) =>
  ...
  // NOTE: we have to use non-raw versions for compatibility with Scala.js
  // Realistically, we don't care about specific NaN shape (or NaN values in general),
  // so we also use non-raw values on the JVM
  // https://github.com/scala-js/scala-js/issues/2327
  /**
    * Add a single Float to this builder.
    */
  def putFloatCanonicalNaN(x: Float)(implicit byteOrder: ByteOrder): this.type =
    putInt(java.lang.Float.floatToIntBits(x))(byteOrder)
//    putInt(java.lang.Float.floatToRawIntBits(x))(byteOrder)

  /**
    * Add a single Double to this builder.
    */
  def putDoubleCanonicalNaN(x: Double)(implicit byteOrder: ByteOrder): this.type =
    putLong(java.lang.Double.doubleToLongBits(x))(byteOrder)
