...
object EBAWriter {
  ...
  def writeBytes(structure: EBAStructure, params: SICKWriterParameters): (ByteString, EBAInfo) = {
    assert(params.tableWriteStrategy != TableWriteStrategy.StreamRepositioning, "StreamRepositioning strategy is not supported by EBAWriter.writeBytes")
    ...
    try {
      ...
      assert(realOffsets.length == tablesSz, s"EBAWriter.writeBytes: table size mismatch length=${realOffsets.length} tablesSz=$tablesSz")
