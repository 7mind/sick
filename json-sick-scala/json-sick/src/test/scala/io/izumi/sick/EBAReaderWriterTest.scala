...
class EBAReaderWriterTest extends AnyWordSpec {
  ...
  "write test" in {
    val allInputs = FileOps
      .walkFiles(in, _.endsWith(".json"))
      .sortBy(_.length)
          ...
          input =>
            ...
            Seq(true, false).flatMap(x => Seq((x, true), (x, false))).flatMap(y => Seq((y._1, y._2, true), (y._1, y._2, false))).foreach {
              case (dedup, dedupPrimitives, avoidBigDecimals) =>
                ...
                val (raw, packedFileInfo) = getTempBytesFromFile(isJs, roIndex, strategy)
                ...
                val readStructure: EBAStructure = EagerEBAReader.readEBAStructure(FileOps.newInputStream(outFile, buffered = false))
                ...
                val (incStructure: EBAStructure, incJson: Json) = {
                  val reader = IncrementalEBAReader.open(FileOps.newInputStream(outFile, buffered = true), eagerOffsets = true)
  ...
  "eager read test #2" in {
    ...
    for (fpath <- inputs) {
      ...
      try {
        val eba: EBAStructure = EagerEBAReader.readEBAStructure(FileOps.newInputStream(fpath.path, buffered = false))
  ...
  protected def getTempBytesFromFile(isJs: false, roIndex: EBAStructure, strategy: TableWriteStrategy): (Array[Byte], EBAWriter.EBAInfo) = {
    val (dataFile, info) = EBAWriter.writeTempFile(roIndex, SICKWriterParameters(strategy))
    val raw =
      try FileOps.readAllBytes(dataFile.toString)
      finally FileOps.delete(dataFile.toString)
    (raw, info)
  }

  protected def getTempBytesFromFile(isJs: true, roIndex: EBAStructure, strategy: TableWriteStrategy)(implicit d: DummyImplicit): (Array[Byte], EBAWriter.EBAInfo) = {
    val (bs, info) = EBAWriter.writeBytes(
      roIndex,
      SICKWriterParameters(strategy match {
        case TableWriteStrategy.StreamRepositioning => TableWriteStrategy.SinglePassInMemory
        case _ => strategy
      }),
    )
    (bs.toArrayUnsafe(), info)
  }
